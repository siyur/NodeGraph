...
class InputManager(object):
    ...
    # Todo register actions in "Config manager"
    ...
    def loadFromData(self, data):
        for action_name, action_variants in data.items():
            for variant in action_variants:
                action_instance = InputAction().fromJson(variant)
                self.registerAction(action_instance)

    def serialize(self):
        result = defaultdict(list)
        for action_name in self.__actions:
            for actionVariant in self.__actions[action_name]:
                result[action_name].append(actionVariant.toJson())
        return result
